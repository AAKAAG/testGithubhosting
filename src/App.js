...
function Items({ w = 0.9, gap = 0.25 }) {
  ...
  return (
    ...
    <ScrollControls infinite horizontal damping={0.51} pages={(width - xW + urls.length * xW) / width} >
      
      <Scroll>
        {urls.map((url, i) => <Item key={i} index={i} position={[i * xW, 3, -2]} scale={[w, 4, 1]} url={url} />) /* prettier-ignore */}
      </Scroll>
    </ScrollControls>
    </Suspense>
...
export const App = () => (
  <Canvas style={{width: "100vw", height:"100vh", position:"fixed"}} gl={{ antialias: false }} dpr={[1, 1.5]} onPointerMissed={() => (state.clicked = null)}>
